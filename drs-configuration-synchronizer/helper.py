...
def process_stack(prompt, stack_name, template, params, creds, region):
    ...
    if does_stack_exist(stack_name, creds):
...
def zipdir(path, zip_name):
    ...
    for dirname, subdirs, files in os.walk("."):
        ...
        if zip_name in files:
            logger.warning("Zip {} exists in {}, removing from archive".format(zip_name, dirname))
...
def boto3_resource(resource, assumed_credentials=None):
    try:
        config = Config(
            retries=dict(
                max_attempts=40
...
def does_stack_exist(stack, assumed_credentials=None):
    try:
        cf = boto3_client('cloudformation', assumed_credentials)
